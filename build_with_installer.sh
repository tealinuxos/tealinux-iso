...
cd "${SCRIPT_DIR}/tealinux-installer"
...
rm -rf src pkg tealinux-installer
# Build installer from source (clones fresh from GitHub, compiles with current system libs)
makepkg -fs --noconfirm
cp tealinux-installer-git-*.pkg.tar.zst "${SCRIPT_DIR}/localrepo/"
...
# Ensure all static packages in localrepo (like paru, os-prober-btrfs) are in the database
repo-add localrepo.db.tar.xz *.pkg.tar.zst
...
GRUB_THEME_DIR="${SCRIPT_DIR}/tealinux/grub/lorem-loader"
AIROOTFS_THEME_DIR="${SCRIPT_DIR}/tealinux/airootfs/usr/share/grub/themes/lorem-loader"
